...
4. Run the database migrations, **in order**. Open the Supabase dashboard → **SQL Editor → New query** for each:
   ...
   - `supabase/migrations/0008_network_health.sql` — adds `get_network_overview()` (dashboard KPIs) and `search_network_contacts()` (one paginated/searchable/filterable query reused by every contact table in My Network), plus a `selected_contact_emails` column on `campaigns` and `verification_jobs` for bulk actions on an explicit selection.
...
- `supabase/migrations/0008_network_health.sql` — `get_network_overview()` and `search_network_contacts()`, the two queries the redesigned My Network page is built on
...
- `src/app/api/contacts/export-selection` — same, but for an arbitrary set of emails (a My Network bulk-action selection) instead of a whole list
- `src/app/(app)/network` — My Network: a database-health overview (KPI cards, recommended actions, search/filter, company cards), not a bare contact table; `[domain]` is one company's focused page. See **How My Network works** below.
- `src/app/(app)/_components/ContactsTable.tsx` — the one paginated, selectable, server-fetched contacts table used everywhere in My Network (a company's expanded contacts, global search/filter results, and the duplicates-review view) — never loads more than one page into the browser
...
My Network is meant to answer "what's the state of my database and what needs attention," not just list contacts — it deliberately isn't a giant table.

- **Everything is deduplicated by email, network-wide.** Every contact across every list you own is grouped by email domain (`swiftit-solutions.com`, etc.) and by lowercased/trimmed email — the same email in three different lists counts once, everywhere on this page. When copies disagree (e.g. verified in one list, still pending in another), a "best" row is picked: a non-`pending_verification` status wins over pending, then the most recently verified/created copy. `get_network_overview()` (KPI cards), `get_network_domain_counts()` (company cards), and `search_network_contacts()` (every contact table) all use this exact same tie-break, so the numbers never disagree with each other.
- **Nothing loads the whole dataset into the browser.** `search_network_contacts()` (migration 0008) does the dedup, search (`name`/`email`/`company`/`domain`, via `ilike`), status filter, and duplicate-detection server-side, and returns a page of rows plus the filtered total count (via a `count(*) over()` window function) in one round trip — that's what backs `ContactsTable.tsx`, which is reused for a company's expanded contact list, the global search/filter results, and the "Review Duplicates" view. Typing in the search box is debounced (400ms) before it re-queries.
- **Recommended Actions are generated from real counts, not hard-coded.** "X need verification," "X are undeliverable," "X possible duplicates" each only render when that count is actually nonzero, sourced from the same `get_network_overview()` call as the KPI cards. "Duplicate" means an email present in more than one of your `contact_lists` — computed with `count(distinct contact_list_id)` per email, not stored anywhere.
- **Three audience scopes for verification and campaigns, not two.** `verification_jobs` and `campaigns` already supported `contact_list_id` (one list) and `company_domain` (one company, every list) — this redesign adds a third: an explicit array of emails (`selected_contact_emails` on campaigns, `selected_emails` on verification_jobs), for the "Verify Selected" / "Add to Campaign" bulk actions on whatever a user has checked in a `ContactsTable`, regardless of which company or list those contacts happen to be in. Same underlying job/send machinery in all three cases — see `src/lib/verification.ts`'s `VerificationScope` and `campaignSend.ts`'s three-way branch.
- **What's intentionally not here**: job title, department, seniority, and location aren't in the `contacts` schema, so they're not filterable or shown anywhere on this page — that's real, not a stub. Campaign engagement only ever shows what's genuinely tracked (sent, bounced, unsubscribed); "opened/clicked/replied" isn't implemented (would need SES open/click tracking) and isn't referenced in the UI. A natural-language "ask your network" search isn't built either — nothing in this app calls an LLM today.
